...
<% 	
	List<GiftHistory> giveGift=(List<GiftHistory>)request.getAttribute("giveGift"); 
...
<%@ include file="/views/common/header.jsp" %>
    ...
    <div id="subMenu">
		...
		<ul class="nav nav-tabs">
	        ...
	        <li class="nav-item">
	            <a class="nav-link active" id="mileageHistory" href="<%=request.getContextPath()%>/giveTicket.do?memberId=<%=loginMember.getMemberId()%>">보낸 이용권</a>
	        </li>
	    </ul>
		<div>
			<table>
				 <tr>
			      	<td colspan='3'>보낸 이용권</td>
			    </tr>
    			<tr>
			        <th>이용권 이름</th>
			        <th>받는 사람</th>	        
    			...
    			<%if(!giveGift.isEmpty()) {
        			for(GiftHistory g : giveGift){%>
        		<tr>
		            <td><%=g.getTicketCode()%></td>
		            <td><%=g.getTakeMemberId()%></td>
		            <td><%=g.getSendDate()%></td>
        		</tr>
			    <%}
			     }else{ %>
