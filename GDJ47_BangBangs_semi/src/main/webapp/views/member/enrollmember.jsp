...
<body>
	<form name="enrollMemberFrm" action="<%=request.getContextPath() %>/enrollmemberend.do" method="POST" class="joinForm" onsubmit="return fn_enrollmemberValidate();">                                                                                           
      ...
      <div class="textForm">
		...
		<input type="button" id="idCkBtn" value="중복확인">
      </div>
	  <div id="idMsg"></div>
      <div class="textForm">
      ...
      <div class="textForm">
		...
		<input type="button" id="sendBtn" value="인증번호 받기">
      </div>
	  <div id="emailMsg"></div>
	  <div class="textForm">
        <input type="text" id="crtfcNo" class="email" placeholder="인증번호 입력" required>
		<input type="button" id="crtfcNoBtn" value="인증번호 확인">
      </div>
		...
		$(()=>{
			...
			$("#idCkBtn").click(e=>{
	    		...
	    		if(!regid.test(id)){
	    			...
	    			idDuplicateFrm.userId.value=id;
